 ...
 *
 * 如果响应尚未提交，则在超时情况下发送503 (SERVICE_UNAVAILABLE)。
 * 从4.2.8开始，这是通过返回{@link AsyncRequestTimeoutException}作为处理的结果间接完成的，然后由Spring MVC的默认异常处理作为503错误处理。
 *
 *
 ...
 * <p>在所有其他拦截器之后注册，因此只有在没有其他拦截器处理超时时才调用。
 *
 ...
 * header if necessary.
 * 请注意，根据RFC 7231，没有'Retry-After'报头的503将被解释为500错误，客户端不应该重试。
 * 应用程序可以安装自己的拦截器来处理超时，并在必要时添加'Retry-After'报头。
 *
 *
