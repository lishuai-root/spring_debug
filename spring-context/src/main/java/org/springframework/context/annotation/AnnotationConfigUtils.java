...
public abstract class AnnotationConfigUtils {
	 ...
	 * 内部管理的Configuration注释处理程序的bean名称。
	 */
	 ...
	 * 内部管理的@EventListener注释处理器的bean名称。
	 */
	...
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors(
		...
		/**
		 * 添加ConfigurationClassPostProcessor后置beanDefinition处理器，
		 * 在invokeBeanFactoryPostProcessors中调用，用于处理配置类beanDefinition
		 */
		if (!registry.containsBeanDefinition(CONFIGURATION_ANNOTATION_PROCESSOR_BEAN_NAME)) {
			RootBeanDefinition def = new RootBeanDefinition(ConfigurationClassPostProcessor.class);
			def.setSource(source);
			/**
			 * 把ConfigurationClassPostProcessor类型beanDefinition注册到beanDefinitionMap中
			 */
	...
	/**
	 * 如果当前bean需要创建作用域代理，为当前bean创建一个代理bean
	 * 就是@Scope注解的ScopedProxyMode属性
	 * @param metadata
	 * @param definition
	 * @param registry
	 * @return
	...
	static BeanDefinitionHolder applyScopedProxyMode(
		...
		// 如果不需要作用域代理，返回原来的beanDefinition
