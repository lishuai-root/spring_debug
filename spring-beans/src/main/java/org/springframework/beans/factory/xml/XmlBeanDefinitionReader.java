...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 通过属性来记录已经加载的资源
		// 以文件为单位，记录的是已经解析过的xml文件封装的Resource对象
		...
		// 从encodedResource对象中获取已经封装的Resource对象，并获取配置文件的输入流
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			 ...
			 * 逻辑处理的核心步骤
			 *
			 * 完成对xml文件的读取，并将document元素解析为BeanDefinition对象
			 *
			 * 将解析好的BeanDefinition对象注册到DefaultListableBeanFactory.beanDefinitionMap中
			 */
	...
	/**
	 * 把xml文件中的bean定义加载成BeanDefinition对象，每个Bean对象对应一个BeanDefinition对象
	 *
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			...
			// 	将document对象解析成BeanDefinition对象，并注册到beanDefinitionMap中
			// 返回注册的数量
