 ...
 * 类型化字符串值的持有者。可以添加到 bean 定义中，以便显式指定 String 值的目标类型，例如集合元素。
 *
 ...
 * 这个持有者将只存储字符串值和目标类型。实际的转换将由 bean 工厂执行。
 *
...
public class TypedStringValue implements BeanMetadataElement {
	...
	/**
	 * Return the type to convert to.返回要转换为的类型。
	 */
	@Nullable
