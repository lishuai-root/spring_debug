...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * Constant that indicates autowiring a constructor.	指示自动装配构造函数的常数。
	 * @see #setAutowireMode
	 */
	 ...
	 *
	 * 从给定的 bean 定义（可能是子项）覆盖此 bean 定义（可能是从父子继承关系复制的父项）中的设置。
	 *
	 * <ul>
	...
	/**
	 * this.beanClass是对象，可以是String也可以是Class
	 *
	 * Return the current bean class name of this bean definition.返回此 bean 定义的当前 bean 类名称。
	 */
	@Override
	@Nullable
	...
	/**
	 * Specify the class for this bean.	指定此bean的类。
	...
	/**
	 * Return whether this definition specifies a bean class.返回此定义是否指定了一个 bean 类。
	 *
	...
	/**
	 * Return if there are property values defined for this bean. 如果为该bean定义了属性值，则返回。
	 * @since 5.0.2
	 */
	@Override
	...
	/**
	 * Validate and prepare the given method override. 验证并准备给定的方法覆盖。
	 * Checks for existence of a method with the specified name,
	 * marking it as not overloaded if none found.
	 * 检查具有指定名称的方法是否存在，如果没有找到，则将其标记为未重载。
	 *
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		// 如果为0表示，要覆盖的方法在bean类中不存在，抛异常
		if (count == 0) {
			throw new BeanDefinitionValidationException(
					"Invalid method override: no method with name '" + mo.getMethodName() +
					"' on class [" + getBeanClassName() + "]");
		}
		// 如果为1表示，要覆盖的方法在bean类中存在，但是还没有覆盖
		else if (count == 1) {
