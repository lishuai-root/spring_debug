...
abstract class AutowireUtils {
	 ...
	 * 针对给定的所需类型解析给定的自动装配值，例如一个 {@link ObjectFactory} 值到它的实际对象结果。
	 *
	...
	public static Object resolveAutowiringValue(Object autowiringValue, Class<?> requiredType) {
		...
		// 如果autowiringValue是ObjectFactory类型，且autowiringValue不是requiredType的实例
		if (autowiringValue instanceof ObjectFactory && !requiredType.isInstance(autowiringValue)) {
			ObjectFactory<?> factory = (ObjectFactory<?>) autowiringValue;

			// 如果autowiringValue所属类是可序列化的，且是一个接口，创建代理对象
		...
		// 如果autowiringValue就是requiredType类型所属类的实例，直接返回
	 ...
	 *
	 * 反射 {@link InvocationHandler} 用于延迟访问当前目标对象。
	 */
	...
	private static class ObjectFactoryDelegatingInvocationHandler implements InvocationHandler, Serializable {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			switch (method.getName()) {
				case "equals":
					// Only consider equal when proxies are identical.仅当代理相同时才考虑相等。
					return (proxy == args[0]);
				case "hashCode":
					// Use hashCode of proxy.使用代理的 hashCode。
