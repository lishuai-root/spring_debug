...
public abstract class BeanUtils {
	 ...
	 * 使用其“主”构造函数（对于 Kotlin 类，可能声明了默认参数）或它的默认构造函数（对于常规 Java 类，期望标准的无参数设置）实例化一个类。
	 *
	 * <p>Note that this method tries to set the constructor accessible
	 * if given a non-accessible (that is, non-public) constructor.
	 * 请注意，如果给定一个不可访问（即非公共）的构造函数，则此方法会尝试设置可访问的构造函数。
	 *
	...
	public static <T> T instantiateClass(Class<T> clazz) throws BeanInstantiationException {
		...
		try {
			...
			// 使用申明的构造函数通过反射实例化，并返回
			return instantiateClass(clazz.getDeclaredConstructor());
		}
		catch (NoSuchMethodException ex) {

			// 如果没有申明构造函数，使用默认构造函数实例化
			Constructor<T> ctor = findPrimaryConstructor(clazz);

			if (ctor != null) {

				// 使用默认的构造函数通过反射实例化，并返回
			...
			if (KotlinDetector.isKotlinReflectPresent() && KotlinDetector.isKotlinType(ctor.getDeclaringClass())) {
				...
				for (int i = 0 ; i < args.length; i++) {
					...
					if (args[i] == null) {
						...
						// 如果参数类型是原始类型，设置原始类型默认值为参数，否则使用null
	 ...
	 * 返回所提供类的主构造函数。对于 Kotlin 类，这将返回与 Kotlin 主构造函数（在 Kotlin 规范中定义）相对应的 Java 构造函数。
	 * 否则，特别是对于非 Kotlin 类，这只会返回 {@code null}。
	 *
	 * @param clazz the class to check
	 * @since 5.0
